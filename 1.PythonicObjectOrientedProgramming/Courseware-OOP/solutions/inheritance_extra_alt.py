...
class Creature:
    ...
    def attack(self, target):
        return self._simple_attack(target, self.damage)

    # _simple_attack() is a "helper method" whose purpose is to be
    # used by other methods. It isn't meant to be used outside of the
    # class.
    #
    # (That's why we put a single underscore at the beginning. This is
    # a naming convention; it'll be explained more towards the end of
    # this course.)
    def _simple_attack(self, target, damage):
        damage -= target.armor
        if damage < 0:
            damage = 0
...
class Goblin(Creature):
    ...
    def attack(self, target):
        if isinstance(target, Orc):
            ...
            damage = self.damage
        return self._simple_attack(target, damage)
